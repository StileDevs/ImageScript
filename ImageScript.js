...
class Image {
  ...
  *iterateWithColors() {
  ...
  getPixelAt(x, y) {
    ...
    return this.__view__.getUint32(
      ((~~y - 1) * this.width + (~~x - 1)) * 4,
      false,
    );
  }
  ...
  __check_boundaries__(x, y) {
    ...
    if (x < 1) throw new RangeError(`${Image.__out_of_bounds__} (x=${x})<1`);
    if (x > this.width)
      throw new RangeError(
        `${Image.__out_of_bounds__} (x=${x})>(width=${this.width})`,
      );
    if (y < 1) throw new RangeError(`${Image.__out_of_bounds__} (y=${y})<1`);
    if (y > this.height)
      throw new RangeError(
        `${Image.__out_of_bounds__} (y=${y})>(height=${this.height})`,
      );
  }
  ...
  __resize__(width, height, mode = Image.RESIZE_NEAREST_NEIGHBOR) {
    if (width === Image.RESIZE_AUTO && height === Image.RESIZE_AUTO)
      throw new Error(
        "RESIZE_AUTO can only be used for either width or height, not for both",
      );
    else if (width === Image.RESIZE_AUTO)
      width = (this.width / this.height) * height;
    else if (height === Image.RESIZE_AUTO)
  ...
  __resize_nearest_neighbor__(width, height) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        image.__view__.setUint32(
          destPos,
          this.__view__.getUint32(srcPos, false),
          false,
        );
      }
    }
  ...
  drawCircle(x, y, radius, color) {
    ...
    for (
      let currentY = Math.max(1, y - radius);
      currentY <= Math.min(y + radius, this.height);
      currentY++
    ) {
      for (
        let currentX = Math.max(1, x - radius);
        currentX <= Math.min(x + radius, this.width);
        currentX++
      ) {
        if ((currentX - x) ** 2 + (currentY - y) ** 2 < radSquared)
          this.__set_pixel__(
            ...
            typeof color === "function"
              ? color(currentX - x + radius, currentY - y + radius)
              : color,
          );
      }
    }
  ...
  cropCircle(max = false, feathering = 0) {
    ...
    for (const [x, y] of this) {
      ...
      if (distanceFromCenter > radSquared) this.bitmap[alphaIdx] = 0;
        ...
        this.bitmap[alphaIdx] *= Math.max(
          0,
          Math.min(
            1,
            1 - (distanceFromCenter / radSquared) * feathering ** (1 / 2),
          ),
        );
    }
  ...
  composite(source, x = 0, y = 0) {
    ...
    for (let yy = 0; yy < source.height; yy++) {
      ...
      for (let xx = 0; xx < source.width; xx++) {
        ...
        const fg = source.__view__.getUint32(
          4 * (xx + yy * source.width),
          false,
        );
        const bg = this.__view__.getUint32(offset, false);

        if ((fg & 0xff) === 0xff) this.__view__.setUint32(offset, fg, false);
        else if ((fg & 0xff) === 0x00)
          this.__view__.setUint32(offset, bg, false);
        else
          this.__view__.setUint32(offset, Image.__alpha_blend__(fg, bg), false);
      }
    }
  ...
  static __alpha_blend__(fg, bg) {
    ...
    const b =
      (alpha * ((fg >> 8) & 0xff) + inv_alpha * ((bg >> 8) & 0xff)) >> 8;
    const g =
      (alpha * ((fg >> 16) & 0xff) + inv_alpha * ((bg >> 16) & 0xff)) >> 8;
    return (
      ((r & 0xff) << 24) |
      ((g & 0xff) << 16) |
      ((b & 0xff) << 8) |
      (Math.max(fa, bg & 0xff) & 0xff)
    );
  }
  ...
  invert() {
    for (const [x, y, color] of this.iterateWithColors())
      this.__set_pixel__(
        x,
        y,
        ((0xffffffff - color) & 0xffffff00) | (color & 0xff),
      );
  ...
  dominantColor(ignoreBlack = true, ignoreWhite = true, bwThreshold = 0xf) {
    ...
    for (let i = 0; i < this.bitmap.length; i += 4) {
      ...
      const [h, s, l] = Image.rgbaToHSLA(...Image.colorToRGBA(color)).map(
        (v) => ~~(v * 0x3f),
      );
  ...
  __rotate_180__() {
    ...
    while (offset < this.bitmap.length)
      this.bitmap.subarray(offset, (offset += 4)).reverse();
  ...
  flipX() {
    let offset = 0;
    const u32 = this.bitmap;
    const width = this.__width__;
    const height = this.__height__;
  ...
  flipY() {
    const u32 = this.bitmap;
    const width = this.__width__;
    const oheight = this.__height__;
    const height = (this.__height__ / 2) | 0;
   ...
   *
   * @param {Image} source The image to overlay
   * @param {number} x The x coordinate to place the source image (default: 0)
   * @param {number} y The y coordinate to place the source image (default: 0)
   * @returns {Image}
   */
  overlayReplace(source, x = 0, y = 0) {
    const b32 = this.bitmap;
    const f32 = source.bitmap;
    const fw = source.__width__;
    const bw = this.__width__;
    const fh = source.__height__;
    const bh = this.__height__;
    const ox = (x > 0 ? 0 : -x) | 0;
    const oy = (y > 0 ? 0 : -y) | 0;
    const top = (y > 0 ? y : 0) | 0;
    ...
    if (0 >= width || 0 >= height) return this;

    for (let yy = 0; yy < height; yy++) {
      const yyoffset = ox + fw * (yy + oy);
      const yoffset = left + bw * (yy + top);
      b32
        .subarray(yoffset, width + yoffset)
        .set(f32.subarray(yyoffset, width + yyoffset));
    }
    ...
    for (let yy = 0; yy < height; yy++) {
      ...
      for (let xx = 0; xx < width; xx++) {
        ...
        if (fa === 0x00) continue;
          ...
          const g =
            (alpha * ((F >> 8) & 0xff) + inv_alpha * ((B >> 8) & 0xff)) >> 8;
          const b =
            (alpha * ((F >> 16) & 0xff) + inv_alpha * ((B >> 16) & 0xff)) >> 8;
          b32[xx + yoffset] =
            (Math.max(fa, (B >> 24) & 0xff) << 24) |
            ((b & 0xff) << 16) |
            ((g & 0xff) << 8) |
            r;
        }
   ...
   *
   * @param {number} radius The blur radius (default: 1)
   * @returns {Image}
   */
  blurBox(radius = 1) {
    if (!radius || radius <= 0) return this;

    const u8 = new Uint8Array(this.__buffer__);
    const width = this.__width__;
    const height = this.__height__;
    const old = u8.slice();
    const divisor = 1 / (1 + radius + radius);

    Image.__blur_box_vertical__(u8, old, width, height, radius, divisor);
    Image.__blur_box_horizontal__(u8, old, width, height, radius, divisor);
  ...
  blurGaussian(radius = 1) {
    ...
    const width = this.__width__ | 0;
    const rc = (a2 + a3) / (1 - b1 - b2);
    const height = this.__height__ | 0;
    const tmp = new Float32Array(4 * Math.max(width, height));

    Image.__blur_gaussian_core__(
      old,
      u8,
      tmp,
      width,
   ...
   * @private
   */
  static __blur_box_horizontal__(u8, old, width, height, radius, divisor) {
    for (let y = 0; y < height; y++) {
   ...
   * @private
   */
  static __blur_box_vertical__(u8, old, width, height, radius, divisor) {
    for (let x = 0; x < width; x++) {
      let ti = x;
      let li = ti;
  ...
  static gradient(colors) {
    ...
    if (positions.length === 0)
      ...
      return (position) => {
        ...
        return gradient(
          (position - positions[0]) / (positions[1] - positions[0]),
        );
      };
    }
    ...
    return (position) => {
      ...
      if (position >= maxDef)
        return gradients[gradients.length - 1].gradient(1);

      for (const gradient of gradients)
        if (position >= gradient.min && position <= gradient.max)
          return gradient.gradient(
            (position - gradient.min) / (gradient.max - gradient.min),
          );
  ...
  async encodeJPEG(quality = 90) {
    return (await jpeglib.init()).encode(
      this.bitmap,
      this.width,
      this.height,
      quality,
    ...
    if (ImageType.isPNG(view)) {
      ...
      if (pixelType === 0) {
        ...
        for (let i = 0; i < buffer.length; i += 4) {
          image.bitmap[i] =
            0xff * (1 - buffer[i] / 0xff) * (1 - buffer[i + 3] / 0xff);
          image.bitmap[i + 1] =
            0xff * (1 - buffer[i + 1] / 0xff) * (1 - buffer[i + 3] / 0xff);
          image.bitmap[i + 2] =
            0xff * (1 - buffer[i + 2] / 0xff) * (1 - buffer[i + 3] / 0xff);
  ...
  static async renderText(
    ...
    if (image.height > layout.maxHeight)
      image.crop(
        ...
        0,
        image.width,
        Math.floor((layoutOptions.lines() / image.height) * layout.maxHeight) *
          (image.height / layoutOptions.lines()),
      );
...
class Frame extends Image {
  ...
  static __convert_disposal_mode__(mode) {
    ...
    if (mode < 0 || mode > 3) throw new RangeError("Invalid disposal mode");
  ...
  constructor(
    ...
    xOffset = 0,
    yOffset = 0,
    disposalMode = Frame.DISPOSAL_KEEP,
  ) {
  ...
  static from(
    ...
    if (!(image instanceof Image)) throw new TypeError("Invalid image passed");
...
class GIF extends Array {
  ...
  constructor(frames, loopCount = -1) {
    ...
    for (const frame of this)
      if (!(frame instanceof Frame))
        throw new TypeError(
          `Frame ${this.indexOf(frame)} is not an instance of Frame`,
        );
  ...
  *[Symbol.iterator]() {
  ...
  slice(start, end) {
    if (end === Infinity) end = this.length;
    const frames = new Array(end - start);
    for (let i = 0; i < frames.length; i++) frames[i] = this[i + start];
  ...
  async encode(quality = 95) {
    const encoder = new (await giflib.init()).Encoder(
      this.width,
      this.height,
      this.loopCount,
    );
  ...
  static async decode(data, onlyExtractFirstFrame = false) {
    ...
    if (ImageType.isGIF(view)) {
      ...
      for (const frame of decoder.frames()) {
        ...
        const f =
          frames[
            frames.push(new Frame(gwidth, gheight, 10 * frame.delay, 0, 0, 3)) -
              1
          ];
        ...
        if (2 === mode) {
          for (let y = 0 | 0; y < height; y++) {
            ...
            for (let x = 0 | 0; x < width; x++) {
              ...
              if (0 === f8[3 + offset8]) t32[x_offset] = u32[x_offset];
  ...
  resize(width, height, mode = Image.RESIZE_NEAREST_NEIGHBOR) {
    for (const frame of this) frame.resize(width, height, mode);
  }
}
...
class TextLayout {
  ...
  constructor(options) {
    ...
    this.wrapHardBreaks =
      typeof wrapHardBreaks === "undefined" ? true : wrapHardBreaks;
...
class ImageType {
  ...
  static isPNG(view) {
    return (
      view.byteLength >= 4 && view.getUint32(0, false) === MAGIC_NUMBERS.PNG
    );
  }
  ...
  static isJPEG(view) {
    return (
      view.byteLength >= 4 &&
      view.getUint32(0, false) >>> 8 === MAGIC_NUMBERS.JPEG
    );
  }
  ...
  static isTIFF(view) {
    return (
      view.byteLength >= 4 && view.getUint32(0, false) === MAGIC_NUMBERS.TIFF
    );
  }
  ...
  static isGIF(view) {
    return (
      view.byteLength >= 4 &&
      view.getUint32(0, false) >>> 8 === MAGIC_NUMBERS.GIF
    );
  }
}
